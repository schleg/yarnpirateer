...
- (BOOL)create {
	BOOL retval = [_slh insertUsingSQLTemplate:"INSERT INTO fiber (name, friendlyName) VALUES (?,?)" andValues:[NSArray arrayWithObjects:name, friendlyName, nil]];
	return retval;
}
...
+ (NSMutableArray *)all {
	...
	if (SQLITE_OK == sqlite3_open([databasePath UTF8String], &database)) {
		...
		const char *sql = "SELECT name, friendlyName, selected FROM fiber";
		...
		if (SQLITE_OK == prepared) {
			while(SQLITE_ROW == sqlite3_step(selectStatement))
			{
				...
				Fiber *fiber = [[Fiber alloc] initWithName:name friendlyName:friendlyName selected:selected];
  				[retval addObject:fiber];
				[fiber release];
			}
...
- (void)setIsselected:(BOOL)_selected {
	...
	if (SQLITE_OK == sqlite3_open([databasePath UTF8String], &database)) {
		...
		const char *sql = "UPDATE fiber SET selected =? WHERE name=?";
...
- (void)read {
	...
	if (sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
		//NSLog(@"Opened database: %@", databasePath);
		sqlite3_stmt *selectStatement;
		const char *sql = "SELECT friendlyName, selected FROM fiber WHERE name =?";
		int prepared = sqlite3_prepare(database, sql, -1, &selectStatement, NULL);
		if(SQLITE_OK == prepared) {
...
- (BOOL)update {
	BOOL retval = [_slh updateUsingSQLTemplate:"UPDATE fiber SET friendlyName =? WHERE name=?" andValues:[NSArray arrayWithObjects:friendlyName, name, nil]];
	return retval;
}

- (BOOL)delete {
	BOOL retval = [_slh deleteUsingSQLTemplate:"DELETE FROM fiber WHERE name=?" andValues:[NSArray arrayWithObjects:name,nil]];
	return retval;
}
